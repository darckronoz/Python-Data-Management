...
querys.append("select FIRST_NAME, LAST_NAME, SALARY from employees order by SALARY desc select FIRST_NAME, LAST_NAME, SALARY from employees order by SALARY asc select FIRST_NAME, LAST_NAME from employees where FIRST_NAME like 'L%'")
...
querys.append("select em.FIRST_NAME from employees em  where REGEXP_LIKE(lower(em.FIRST_NAME), '[i, o, u]+s')")

for i in range(len(querys)):
    df = pd.read_sql_query(querys[i], engine)
    print(df)